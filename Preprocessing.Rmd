...
source("survey_415684_R_syntax_file.R")

# Read in csv data, group by user_id and keep rows with highest last_page (people that at least started the survey)
csv_data <- read.csv("raw_data/twitter_data.csv") %>% group_by(user_id) %>% filter(last_page != -1, last_page == max(last_page)) %>% ungroup()
...
saveRDS(translated_data, "data/translated_data.rds")
