...
dfr <- data.frame(
  ...
  start.date = as.Date(c("2023-07-19", "2020-03-01", 
                         ...
                         "2023-07-18", "2020-03-01",
                        "2020-12-27", 
                         "2022-05-01", "2022-12-01", "2023-04-01", "2020-03-01",
  ...
  type = c("Collection", "Period", "Collection", "Period", "Collection", "Period", "Collection", "Collection", "Collection", "Collection", "Period", "Collection", "Period")
)
...
timelineplot <- ggplot(dfr %>% filter(source != "RKI")) +
      ...
      theme_minimal() +
      scale_color_manual(values = palette_twittermastodonsurvey_bars()) +
      ylab("") +
...
timelineplot2 <- ggplot(dfr %>% filter(source %in% c("External Survey", "RKI", "MuSPAD"))) +
      ...
      scale_x_date(breaks= seq(as.Date("2020-01-01"), as.Date("2023-08-31"), by = "6 months"), date_labels = "%Y/%m")+
      theme_minimal() +
      scale_color_manual(values = palette_twittermastodonsurvey_bars()) +
      ylab("") +
      ggtitle("Data Collection/Infection Occurence Timeline") +
