...
HouseholdDataCloseContact <- HouseholdData %>% pivot_longer(cols = c("cc_hsld_size_pre_pandemic_2019_num_hsld_members", "cc_hsld_size_pre_pandemic_03_2020_num_hsld_members", "cc_hsld_size_pre_pandemic_summer_2021_num_hsld_members", "cc_hsld_size_pre_pandemic_01_2023_num_hsld_members"))
HouseholdDataCloseContact$name <- factor(HouseholdDataCloseContact$name, levels = c("cc_hsld_size_pre_pandemic_2019_num_hsld_members", "cc_hsld_size_pre_pandemic_03_2020_num_hsld_members", "cc_hsld_size_pre_pandemic_summer_2021_num_hsld_members", "cc_hsld_size_pre_pandemic_01_2023_num_hsld_members"))
HouseholdDataCloseContact$value <- as.integer(HouseholdDataCloseContact$value)
ggplot(HouseholdDataCloseContact %>% filter(value < 10)) + geom_bar(aes(x= value, y = ..prop.., group = 1, fill = name)) +
  theme_minimal() +
  facet_wrap(~name, nrow=2) +
  ylab("Frequency") +
  xlab("") +
...
#ggsave("HouseholdSizeCC.png", dpi = 500, w = 9, h = 5)
...
ggplot(SchoolDataRespondent %>% filter(value <= 100)) + geom_histogram(aes(x = value, y = after_stat(count / sum(count)), fill = name), bins = 10) +
  ...
  labs(subtitle=paste0("#close contacts with more than 100 work contacts: ", as.character(nrow(SchoolDataRespondent %>% filter(value > 100))), "\nMax # of work contacts (in 2019): ", as.character(max(SchoolDataRespondent$value, na.rm=TRUE))))
#ggsave("WorkContactsRespondent.png", dpi = 500, w = 9, h = 5)
...
SchoolDataCCPre <- SchoolDataCCPre %>% mutate(name = case_when(name == "cc_weekly_contacts_2019_school_kinder" ~ "CC's School contacts 2019",
...
SchoolDataCCDuring <- SchoolDataCCDuring %>% mutate(name = case_when(name == "cc_weekly_cont_during_pandemic_2019_school_kinder" ~ "CC's School contacts 2019",
                                                               name == "cc_weekly_cont_during_pandemic_03_2020_school_kinder_cont"~ "CC's School contacts 03/20",
                                                               name == "cc_weekly_cont_during_pandemic_2021_school_kinder_cont"~  "CC's School contacts summer 21",
                                                               name == "cc_weekly_cont_during_pandemic_01_2023_school_kinder_cont"~ "CC's School contacts 01/23"))
...
LeisureDataCCDuring <- LeisureData %>% select(user_id, cc_weekly_cont_during_pandemic_2019_leisure_cont, cc_weekly_cont_during_pandemic_03_2020_leisure_cont, cc_weekly_cont_during_pandemic_summer_2021_leisure_cont, cc_weekly_contacts_01_2023_leisure_cont) %>%
                                              pivot_longer(cols = c("cc_weekly_cont_during_pandemic_2019_leisure_cont", "cc_weekly_cont_during_pandemic_03_2020_leisure_cont", "cc_weekly_cont_during_pandemic_summer_2021_leisure_cont", "cc_weekly_contacts_01_2023_leisure_cont"))
