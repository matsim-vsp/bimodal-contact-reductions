...
WorkDataRespondent <- WorkDataRespondent %>% mutate(name = case_when(name == "wkly_cont_2019_work_uni" ~ "Work2019",
                                                                     name == "wkly_cont_03_2020_work_uni"~ "Work03/20",
                                                                     name == "wkly_cont_summer_2021_work_uni"~ "WorkSummer 21",
                                                                     name == "wkly_cont_01_2023_work_uni"~ "Work01/23"))
WorkDataRespondent$name <- factor(WorkDataRespondent$name, levels = c("Work2019", "Work03/20", "WorkSummer 21", "Work01/23"))
WorkDataRespondent$value <- as.integer(WorkDataRespondent$value)

WorkDataRespondent <- WorkDataRespondent %>% filter(!is.na(value)) %>% mutate(time = case_when(name == "Work2019" ~ "2019",
                                                                                              name == "Work03/20" ~ "03/2020",
                                                                                              name == "WorkSummer 21" ~ "Summer 21",
                                                                                              name == "Work01/23" ~ "01/2023")) %>%
                                                                      mutate(context = "work")
...
LeisureDataRespondent <- LeisureDataRespondent %>% filter(!is.na(value)) %>% mutate(time = case_when(name == "weekly Leisure contacts 2019" ~ "2019",
                                                                                               name == "weekly Leisure contacts 03/20" ~ "03/2020",
                                                                                               name == "weekly Leisure contacts summer 21" ~ "Summer 21",
                                                                                               name == "weekly Leisure contacts 01/23" ~ "01/2023")) %>%
...
data_full <- rbind(data_full, all_contacts)
...
all <- ggplot(data_full %>% filter(value < 250) %>% filter(context == "all Contacts") %>% filter(!is.na(time)), aes(time, value)) +
  geom_boxplot(color = "#29335C") +
  #facet_wrap(~time) +
  theme_minimal() +
  scale_color_manual(values = palette()) +
