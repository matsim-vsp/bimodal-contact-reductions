...
data_first_and_second_resp <- data_first_and_second_resp %>% mutate(context = case_when(str_detect(name, "hsld") ~ "Household",
                                                                                     str_detect(name, "school") ~ "School",
                                                                                     str_detect(name, "work") ~ "Work",
...
ggplot(data_first_and_second_resp %>% filter(str_detect(name, "diff")) %>% filter(value < 15) %>% filter(value > -15)) + 
  geom_histogram(aes(x = value, y = after_stat(count / sum(count)), fill = context), bins = 10) +
  theme_minimal() +
  facet_wrap(~name, nrow=2) +
...
ggsave("DifferenceFirstSecondComparison.pdf", dpi = 500, w = 15, h = 4.5)
ggsave("DifferenceFirstSecondComparison.png", dpi = 500, w = 15, h = 4.5)