...
filter(cc_pre_leisure_01_2023 != -1000)) + 
  ...
  ggplot(data_reduced %>% 
mutate(respondent_leisure_01_2023 = case_when(respondent_leisure_01_2023 == 0 ~ 0.01, .default = respondent_leisure_03_2020)) %>%
mutate(cc_pre_leisure_01_2023 = case_when(cc_pre_leisure_01_2023 == 0 ~ 0.01, .default = cc_pre_leisure_03_2020)) %>%
mutate(RiskyCarefulAtt = case_when(is.na(RiskyCarefulAtt) ~ "No Risk Perception Score", 
.default = RiskyCarefulAtt)) %>%
filter(respondent_leisure_03_2020 != -1000) %>% 
filter(cc_pre_leisure_03_2020 != -1000)) +
  facet_wrap(~RiskyCarefulAtt) +
  geom_jitter(aes(x=respondent_leisure_01_2023, y = cc_pre_leisure_01_2023, color = RiskyCarefulAtt), size = 3, alpha = 0.7) +
  theme_minimal() +
...
filter(cc_pre_leisure_01_2023 != -1000)) + 
  ...
  theme(text = element_text(size = 30)) +
  my_theme() +
  guides(color = guide_legend(nrow=2), legend.position = "bottom") +
  theme(plot.title = element_text(hjust = 0.5)) 
